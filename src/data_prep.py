...
def albp_to_features_nn(alb_instance, salbp_type="salbp_1", cap_constraint=None, G_max_red=None, G_max_close=None, n_random=100, n_edge_random=100, feature_types={"all"}, return_assignments = False):
    ...
    if salbp_type == "salbp_1":
        ...
        t_start = time.perf_counter()
        final_data = get_combined_edge_and_graph_data_neural_net(alb_instance, graph_data, G_max_close=G_max_close, feature_types=feature_types, n_random_solves=n_edge_random)
...
def main():
    ...
    # Process the range
    folder = os.path.dirname(args.output_fp)
